...
def expression_pipeline(
    ...
    for chromosome in chromosomes.split(","):
        for cell_type in celltypes.split(","):
            expression_adata = get_chrom_celltype_expression(
                gene_info_df, expression_files_prefix, chromosome, cell_type
            )
            filtered_adata = filter_lowly_expressed_genes(expression_adata, min_pct_expr)
            for gene in filtered_adata.var_names:
                build_pheno_cov_filename(
                    gene,
                    expression_adata,
                    get_celltype_covariates(expression_files_prefix, cell_type),
                    sample_mapping_file_path,
                )
